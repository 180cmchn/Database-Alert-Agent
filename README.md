...
一个可插拔、可审计的数据库告警调查框架。它通过 HTTP 或 Kafka 接收告警后，同时启动两条互不阻塞的链路：AI 异步调查，以及按版本化策略执行的等待、通知、确认、升级和恢复流程。
...
```text
HTTP / Kafka 接入 → 标准化、三等级映射、脱敏、持久化
                         │
           ┌─────────────┴─────────────┐
           │                           │
  立即进入 AI 调查队列             首次命中路由策略
  指纹/网页手册/只读工具           60/300 秒持久化等待
  建议/验收/人工反馈               群通知/卡片/电话升级
           │                           │
           └─────────────┬─────────────┘
                         │
                    统一审计存储
...
- `AlertSignalRouter`：处理与 AI 无关的告警分派、确认、恢复和升级生命周期。
...
`runbooks/` 下的 Markdown 仅作为公司内网/VPN 网页手册索引；本地正文不参与匹配，也不存在
本地回退。命中候选后，服务携带 Cookie 或 Bearer 会话读取权威网页正文。配置与格式见
`runbooks/README.md`。对于需要执行 JavaScript、验证码或交互式 SSO 登录的页面，应使用手册
系统提供的正文导出/API URL，或另行实现专用 `RunbookProvider`；服务不会保存公司账号密码。
...
[`policies/alert-routing.yaml`](policies/alert-routing.yaml) 是路由事实来源。列表顺序就是优先级，系统自上而下匹配并在第一条命中后停止；文件内的 `priority` 必须严格递减，否则服务启动失败。每个事件会保存命中策略快照，因此 Git 中后续修改策略不会改变已经进入等待期的事件。

当前策略顺序与流程图一致：MySQL DATA 复制延迟、Wukong OceanBase 测试、SC-OB Critical、非工作时间 Critical、通用 Critical/Warning、Info。Wukong 首次只发 `TEST_DATABASE` 群，未确认后仍可升级到生产群、卡片，Critical 可继续电话升级。

路由等待写入 `alert_incidents`，投递结果写入 `escalation_deliveries`，不是进程内 `sleep`。企微卡片确认会调用 `POST /api/v1/integrations/wecom/alerts/{incident_id}/ack`；电话接口返回 `{ "connected": true }` 也视为确认。确认只停止通知升级，AI 调查继续运行。值班表 API 不可用或未配置时，值班人回退为 `sona`。

公司接口确定后在环境中填写，YAML 只保存逻辑目标，不保存密钥或实际 URL：

```dotenv
ALERT_ROUTING_POLICY_PATH=./policies/alert-routing.yaml
ALERT_GROUP_WEBHOOK_URLS={"DATA":"https://...","TEST_DATABASE":"https://...","DATABASE_ALERTS":"https://...","DATABASE_INFO":"https://...","SC_OB":"https://..."}
WECOM_CARD_API_URL=https://internal.example/wecom/cards
WECOM_CARD_API_BEARER_TOKEN=replace-me
WECOM_ACK_CALLBACK_BASE_URL=https://alert-agent.internal.example/
WECOM_ACK_CALLBACK_TOKEN=replace-me
WECOM_ONCALL_API_URL=https://internal.example/wecom/oncall
WECOM_ONCALL_API_BEARER_TOKEN=replace-me
PHONE_NOTIFICATION_API_URL=https://internal.example/phone/calls
PHONE_NOTIFICATION_API_BEARER_TOKEN=replace-me
FALLBACK_ONCALL=sona
```

值班表草案响应字段为 `on_call` 和 `is_working_time`；电话草案响应字段为 `connected`。在公司 API 契约最终确定前，这些适配器会明确记录为未配置/投递失败，不会伪造成功。
...
AI 通知阶段包括 `ADVICE_READY` 和 `ANALYSIS_FAILED`。原始告警首发已经由上面的路由策略负责，不再由 AI 流程即时发送。默认只有 `CRITICAL` 在调查成功后补发建议，失败则补发需人工介入状态。通知失败会按配置重试并写入审计记录，但不会丢弃已经生成的分析结果。
...
[`tools/qq_mail_relay/README.md`](tools/qq_mail_relay/README.md)。四类告警信号和自动
...
- `POST /api/v1/admin/incidents/{id}/ack`：管理员手工确认告警并停止后续升级。
