...
只读，但最终建议不受只读限制：建立根因后，主 Agent 将建议处理结果分别写入“临时解决”和“长期优化”
两个独立数组。临时解决用于控制当前影响、恢复服务或降低当前风险；长期优化用于消除已证实根因或降低
同类问题复发概率。两类建议都必须基于现有证据，并说明目标、执行前提、预期结果、风险、审批、停止
条件或回滚要求；系统只生成建议，不执行动作，也不让 DBA 重复 MCP 已完成的核查。证据只支持一类时
另一类保持为空；无法建立根因时两个数组都为空，不生成猜测性建议。
