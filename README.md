...
4. 轮询器按 `created_at` 时间窗调用 `/alert/list`，对每条记录优先调用 `/alert/info`，并以 `source + alert_id` 幂等入库；重复的 `alert_id` 不会创建第二个分析任务。系统不会因同一告警后续状态更新而重复分析。
...
结果经脱敏和长度限制后作为实时证据回传给下一轮模型调用；其中的自然语言仅作为结果内容，
不能覆盖任务或安全规则。模型最多执行 10 个只读步骤。`apply_query_permission_gymJPA` 等会产生
外部状态变更的工具不会传给模型。

Host 不再重复实现各资源发现工具的参数规则；登录、资源组、实例、数据库、表和字段调用直接使用
MCP 实时输入 Schema，由模型结合前序结果自主规划，并由 MCP 服务端校验。Host 仅保留只读工具
白名单以及最终 SQL 查询的登录状态、实例标识、目标数据库、SQL、时间窗和结果上限校验。
模型不能扩大时间范围、改写 SQL、切换数据库或调用权限申请工具。查询结构固定，但时间边界由
规范化告警的 `occurred_at` 和部署窗口计算。默认窗口与慢查询告警规则一致，为截至告警发生时的
前 5 分钟：
...
查询结果以 `source_system=archery_mcp` 的实时 `EvidenceRecord` 保存并传给 Agent。查询请求按
告警时间窗构造，但实际执行 SQL 未校验；当前表查询也尚未按告警指向的受影响数据库实例过滤，
并受 20 行和 8,000 字符上限约束。因此结果标记为可能截断且不可单独用于根因判定的排查线索。
