...
模型供应商出现确认的认证、授权、额度/计费或模型配置故障时，系统只暂停分析 dispatch。轮询器仍按原
时间窗完整拉取、去重并入库，但不会把新告警送入 Worker；这些告警保持 `RECEIVED`/`QUEUED`。最近一次
轮询的开始/完成时间、窗口、拉取/新增/去重计数或脱敏失败信息持久化，并显示在 Agent 设置页。

重启、下一轮轮询成功或保存模型设置都不会恢复 dispatch。管理员需要在设置页显式执行“验证模型并恢复
调度”；验证带当前配置指纹和 dispatch version，成功后仅补投 `RECEIVED`/`QUEUED`。历史 `FAILED`
告警不会由轮询或恢复动作重跑，只能逐条显式 reanalysis。
